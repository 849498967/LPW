    ...
    for each_lot in lot_list:
        ...
        for each_sh_folder in sh_folder_list:
            ...
            if match_res:
                ...
                # 869 fail pattern
                # DUT07 BANK1 CHIP05 PLANE01 TB446 FAILBLOCK 0x4001
                pattern = re.compile(r".*?(DUT\d+) (BANK\d+) (CHIP\d+) (PLANE\d+) TB446 FAILBLOCK 0x\d+")
                ...
                for each_site in sh_log_list:
                    ...
                    with open(sh_log_path + "\\" + each_site, "r") as f:
                        ...
                        for log in f:
                            ...
                            if re.match("  xml file  Detected  in XMLFILE folder for Hardware", log):
                                touch_down += 1
                            ...
                            if match_869:
                                ...
                                no_869_fail_but_has_553_fail_flag = 0
                                tb__869_detect = 1
                                [dut, bank, chip, plane] = [match_869.group(1), match_869.group(2),
                                ...
                                if [dut, bank, chip] not in die_temp_list:
                                    die_temp_list.append([dut, bank, chip])
                                print("find fail die: ", dut, bank, chip, plane)
                                key = "_".join(["SITE"+str(touch_down), dut, bank, chip])
                                # print(key)
                            ...
                            if tb__869_detect:
                                ...
                                for each_die in die_temp_list:
                                    [dut, bank, chip] = each_die
                                    # DUT01 CHANNEL2   CHIP01		17		11		08		DP1584825
                                    wxyl_pattern = re.compile(r"DUT%s %s (CHIP%s) {6}(.*)		(.*)		(.*)		(.*) 	CH\d"
                                                              % (dut[3:], bank, chip[4:]))
                                    ...
                                    if wxyl_match:
                                        ...
                                        if key not in tb__869_dict:
                                            ...
                                            print("No WXY find!")
                                            temp = touch_down - 1
                                            temp_key = "_".join(["SITE" + str(temp), dut, bank, chip])
